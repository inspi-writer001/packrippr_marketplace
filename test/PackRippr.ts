...
describe("PackripprMarketplace", function () {
  ...
  beforeEach(async function () {
    // getSigners is available directly on the ethers object imported from hardhat
    [owner, user, feeRecipient] = await ethers.getSigners();

    // 1. Deploy Mock ERC20
    const ERC20Factory = await ethers.getContractFactory("MockERC20");
    paymentToken = (await ERC20Factory.deploy("USDC", "USDC")) as MockERC20;

    // 2. Deploy NFT Contract (Note: using 'PackripprNFT' name from your tree)
    ...
    )) as PackripprMarketplace;

    // 4. Setup permissions
    await marketplace.setAllowedPaymentToken(
    ...
    await paymentToken
      .connect(user)
      .approve(await marketplace.getAddress(), parseEther("1000"));
  });
  ...
  describe("Trading Flow", function () {
    ...
    it("Should execute a purchase and distribute fees correctly", async function () {
      const sellerBalanceBefore = await paymentToken.balanceOf(
        await owner.getAddress()
      );
      ...
      expect(await paymentToken.balanceOf(await owner.getAddress())).to.equal(
        sellerBalanceBefore + parseEther("97.5")
      );
      expect(
        await paymentToken.balanceOf(await feeRecipient.getAddress())
      ).to.equal(feeRecipientBalanceBefore + parseEther("2.5"));
    });
  });
  ...
  describe("Offer System", function () {
    it("Should allow users to create and owners to accept offers", async function () {
      ...
      const userBalanceBefore = await paymentToken.balanceOf(
      ...
      expect(await nft.ownerOf(0)).to.equal(await user.getAddress());
      expect(await paymentToken.balanceOf(await user.getAddress())).to.equal(
        userBalanceBefore - parseEther("50")
      );
    });
  });
  ...
  describe("Listing Management", function () {
    it("Should allow users to cancel their own listings", async function () {
      ...
      await marketplace
        .connect(user)
        .createListing(await nft.getAddress(), 0, ZeroAddress, parseEther("1"));

      await expect(marketplace.connect(user).cancelListing(1)).to.emit(
        marketplace,
        "ListingCancelled"
      );
    });
    ...
    it("Should allow users to update their own listing prices", async function () {
      ...
      await expect(
        marketplace.connect(user).updateListingPrice(1, parseEther("2"))
      )
